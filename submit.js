...
// const resultFile = __dirname + '/tests/result.json';
...
exec("npm run test" + hwNumber, async (error, stdout, stderr) => {
  ...
  const seguir = prompt('Estas seguro que deseas submitear este resultado? (S/n) ').toLowerCase();
  if (seguir !== 's') {
    return process.exit(0);
  }

  const username = prompt('Usuario de github:').toLowerCase();
  ...
  try {
    ...
    await ejecutar(`git push https://${username}:${password}@github.com/${username}/${repo} master`);
  } catch (e) {
    console.error(e);
    return process.exit(0);
  }

  fetch('https://proxy.soyhenry.com:3000/submit', {
    method: 'post',
    headers: {
